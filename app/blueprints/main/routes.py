...
def before_app_request():
    g.config = get_raspimjpeg_config({})
    g.extra = {}
...
def login():
    ...
    if request.method == "POST":
        ...
        if pwd and (pwd2 := request.form.get("password_2")):
            ...
            elif user == get_settings().get("users", {}).get(user):
            ...
            if login := get_settings().get("users", {}).get(user):
                hashed = hashlib.pbkdf2_hmac(
                    "sha256", pwd.encode("utf-8"), "mymy".encode("utf-8"), 100000
                )
    ...
    if get_settings().get("users") is None:
        init_settings()
        init = True
...
def index():
    ...
    mode = 0
    cam_pos = None
    if file_exists(f"{rpath}/pipan_on"):
        pipan = file_get_content(f"{rpath}/pipan_on")
        cam_pos = pipan.split(" ")
        mode = 1
    if get_settings().get("servo_on"):
        mode = 2

    set_settings({"mode": mode})

    user_buttons = get_settings().get("user_buttons", [])
    preset = get_settings().get("upreset", "v2")
...
def log():
    ...
    if request.method == "POST" and (action := request.form.get("action")):
        match action:
            case "downloadlog":
                filename = current_app.config["LOGFILE_SCHEDULE"]
                return send_file(f"{rpath}/{filename}", as_attachment=True)
            case "clearlog":
                filename = current_app.config["LOGFILE_SCHEDULE"]
                os.remove(f"{rpath}/{filename}")
...
def schedule():
    rpath = current_app.config.root_path
    filename = current_app.config["SCHEDULE_CONFIG"]
    bk_config = current_app.config["SCHEDULE_CONFIGBACKUP"]
    ...
    if request.method == "POST" and (action := request.form.get("action")):
        match action:
            ...
            case "backup":
                ...
                settings = get_settings()
                set_settings(settings, f"{rpath}/{bk_config}")
            case "restore":
                write_log("Restored up schedule settings")
                settings = get_settings(f"{rpath}/{bk_config}")
                set_settings(settings, f"{rpath}/{filename}")

    settings = get_settings()

    offset = get_time_offset(settings.get(SCHEDULE_GMTOFFSET))

    sunrise = get_sunrise(
        settings.get(SCHEDULE_LATITUDE),
        settings.get(SCHEDULE_LONGITUDE),
        offset,
    )
    sunset = get_sunset(
        settings.get(SCHEDULE_LATITUDE),
    ...
    period = day_period(
        ...
        sunrise=sunrise,
        sunset=sunset,
        day_mode=settings.get(SCHEDULE_DAYMODE),
        daw=settings.get(SCHEDULE_DAWNSTARTMINUTES),
        day_start=settings.get(SCHEDULE_DAYSTARTMINUTES),
        dusk=settings.get(SCHEDULE_DUSKENDMINUTES),
        day_end=settings.get(SCHEDULE_DAYENDMINUTES),
...
def status_mjpeg():
    ...
    file_content = ""
    for i in range(0, 30):
        file_content = file_get_content(f"{rpath}/status_mjpeg.txt")
        if file_content != request.args.get("last"):
            break
        time.sleep(0.1)
    os.popen(f"touch {rpath}/status_mjpeg.txt")
...
def pipan():
    ...
    if action := request.args.get("action"):
        try:
            input = json.loads(file_get_content(f"{rpath}/{SERVO_DATA}"))
        ...
        file_set_content(f"{rpath}/{SERVO_DATA}", json.dumps(servoData))
